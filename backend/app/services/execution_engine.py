"""
Execution Engine v3 — Integrated with real architecture.

Uses:
- app.services.llm_service.model_service for LLM calls (shared clients, connection pooling)
- app.tools.registry.ToolRegistry for tool access (the real registry)
- Budget-controlled context flow between steps
- Type-aware filtering
- Memory injection into synthesis
"""
...
def enrich_tool_input(
    ...
    if context_text:
        ...
        injected_keys.extend(["previous_output", "use_previous_output"])
...
class ExecutionEngine:
    ...
    async def _synthesis_call(
        ...
        """Use the shared model_service for synthesis calls.
        
        model_service handles provider fallback, connection pooling,
        and error handling. No need to duplicate that here.
        """
        messages = [
        ...
        try:
            return await model_service.chat(messages=messages, max_tokens=max_tokens)
        except Exception:
            return None
        ...
        for accumulated in execution_chain:
            if total_context_chars > MAX_SYNTHESIS_CONTEXT_CHARS:
                step_descriptions.append(
                    f"Step {accumulated.step_index}: [context budget exhausted, truncated]"
                )
                break
    ...
    async def _execute_single_step(
        ...
        try:
            ...
            if not isinstance(raw_result, dict):
                raw_result = {"ok": True, "content": str(raw_result)}
    ...
    async def execute_plan(
        ...
        if not planner_result.is_multi_intent or not planner_result.steps:
            ...
            try:
                ...
                return ExecutionResult(
                    is_multi_intent=False, trace=trace, error=str(exc)
                )
        ...
        for i, step in enumerate(steps):
            ...
            normalized = (
                _normalize_tool_output(step_result.raw_output)
                if step_result.raw_output
                else {}
            )
            execution_chain.append(AccumulatedStep(
                step_index=i, intent=step.intent, tool=step.tool,
        ...
        if (has_chat_step or has_tool_output) and has_tool_output:
            ...
            started = time.perf_counter()
            final_response = await self._synthesis_call(
            ...
            if final_response:
                ...
                parts = [
                    c.output.content
                    for c in tool_steps
                    if c.output.ok and c.output.content
                ]
