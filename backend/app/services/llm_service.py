...
# Shared persistent clients — one per provider, reused across all requests
...
class BaseChatProvider:
    ...
    async def stream(
        self, messages: list[dict[str, str]], max_tokens: int = 1200
    ) -> AsyncIterator[str]:  # Changed to yield strings for orchestrator compatibility
        raise NotImplementedError
...
class ZAIChatProvider(BaseChatProvider):
    ...
    def __init__(self, model: str = "glm-4.5-flash") -> None:
        self.model = model
        self.url = "https://api.z.ai/api/paas/v4/chat/completions"
        self._client = _openai_client
