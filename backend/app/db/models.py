...
class User(Base):
    ...
    public_id: Mapped[str] = mapped_column(String(36), unique=True, default=lambda: str(uuid4()), index=True)
...
class Conversation(Base):
    ...
    active_mode: Mapped[ConversationMode] = mapped_column(Enum(ConversationMode), default=ConversationMode.NORMAL_CHAT)
...
class Turn(Base):
    ...
    assistant_message_id: Mapped[int | None] = mapped_column(ForeignKey("messages.id"), nullable=True)
    ...
    status: Mapped[TurnStatus] = mapped_column(Enum(TurnStatus), default=TurnStatus.STARTED)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)
    completed_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)

    conversation: Mapped["Conversation"] = relationship(back_populates="turns")
    messages: Mapped[list["Message"]] = relationship(back_populates="turn", foreign_keys="Message.turn_id")
    ...
    __table_args__ = (
        UniqueConstraint("conversation_id", "sequence_number", name="uq_turn_sequence_per_conversation"),
    )
...
class Message(Base):
    ...
    public_id: Mapped[str] = mapped_column(String(36), unique=True, default=lambda: str(uuid4()), index=True)
    conversation_id: Mapped[int] = mapped_column(ForeignKey("conversations.id"), index=True)
    turn_id: Mapped[int | None] = mapped_column(ForeignKey("turns.id"), nullable=True, index=True)
...
class ToolEvent(Base):
    ...
    status: Mapped[ToolStatus] = mapped_column(Enum(ToolStatus), default=ToolStatus.PENDING, index=True)
...
class Artifact(Base):
    ...
    conversation_id: Mapped[int] = mapped_column(ForeignKey("conversations.id"), index=True)
    turn_id: Mapped[int | None] = mapped_column(ForeignKey("turns.id"), nullable=True, index=True)
    source_tool_event_id: Mapped[int | None] = mapped_column(ForeignKey("tool_events.id"), nullable=True)
